...
import '../../core/local_auth/local_session_state.dart';
...
/// Auth gating lives in [redirect], driven by [LocalSessionController]:
///  - while the local session is restoring ([LocalAuthStatus.unknown]) →
///    hold in place (no route matches yet, so nothing renders twice),
...
final locationAvailabilityInquiryRouterProvider = Provider<GoRouter>((ref) {
  final refresh = ValueNotifier<LocalAuthStatus>(LocalAuthStatus.unknown);
  ref.onDispose(refresh.dispose);
  ref.listen(
    localSessionControllerProvider.select((s) => s.status),
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      final status = ref.read(localSessionControllerProvider).status;
      if (status == LocalAuthStatus.unknown) return null;

      final loggingIn = state.matchedLocation == LocationAvailabilityInquiryRoute.loginPath;
      if (status == LocalAuthStatus.unauthenticated) {
